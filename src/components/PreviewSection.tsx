...
const PreviewSection: React.FC<PreviewSectionProps> = ({ style }) => {
  ...
  const renderKey = `${style.enableGradient}-${style.gradientDirection}-${style.gradientStart}-${style.gradientEnd}-${style.textColor}-${style.segments.length}-${style.enableLetterAnimation}-${style.letterAnimationPattern}`;
  ...
  useEffect(() => {
    ...
    // Apply letter animations with patterns
    if (style.enableLetterAnimation) {
  ...
  return (
    <div className="lg:h-[75vh] bg-gray-50 dark:bg-gray-800 rounded-lg flex-1 flex items-center justify-center mb-6">
      <div
        ref={previewRef}
        className="p-8 text-center max-w-full break-words cursor-pointer"
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        <div ref={textRef} key={renderKey} style={baseStyle}>
          {renderTextContent()}
        </div>
      </div>
    </div>
